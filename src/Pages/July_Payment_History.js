...
import data from './July_Payments.json';
import { jsPDF } from 'jspdf';
import autoTable from 'jspdf-autotable';

function JulyPaymentStatusList() {
  ...
  useEffect(() => {
    localStorage.setItem('julyConsumerList', JSON.stringify(list));
  }, [list]);
  ...
  const downloadData = () => {
    ...
    link.href = url;
    link.download = 'July_Consumer_List.json';
    link.click();
  ...
  const resetToFreshData = () => {
    localStorage.removeItem('julyConsumerList');
    setList(data);
  };

  return (
    <div className="max-w-6xl mx-auto p-4 bg-white shadow-md rounded">
      <h1 className="text-xl font-bold mb-4">💸 July Payment Status List</h1>
...
export default JulyPaymentStatusList;
